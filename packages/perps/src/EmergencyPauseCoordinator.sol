...
/// @notice Allows one guardian to atomically stop new trading risk and LP entry across the perps stack.
/// @dev The coordinator can only add restrictions. It cannot unpause either component, change their configuration,
///      set prices, move funds, or make arbitrary calls. Governance retains recovery authority directly on each
///      component. The reason and evidence hashes are advisory incident metadata and are not interpreted on-chain.
contract EmergencyPauseCoordinator is Ownable2Step {
    ...
    /// @notice Bit identifying the HousePool pause state in event masks.
    uint8 public constant HOUSE_POOL_PAUSED_MASK = 1 << 1;
    /// @notice Mask emitted after every successful trigger.
    uint8 public constant ALL_COMPONENTS_PAUSED_MASK = ROUTER_ADMIN_PAUSED_MASK | HOUSE_POOL_PAUSED_MASK;
    ...
    /// @notice HousePool component whose pause blocks new LP entry.
    IPerpsAdmin public immutable HOUSE_POOL;
    ...
    /// @notice Emitted after both component pause gates are confirmed active in one atomic transaction.
    ...
    /// @param previousPauseMask Component pause mask observed before containment
    /// @param newPauseMask Component pause mask after containment
    ...
    constructor(
        ...
        HOUSE_POOL = IPerpsAdmin(housePool);
    }
    ...
    function triggerEmergencyPause(
        ...
        bool routerWasPaused = ROUTER_ADMIN.paused();
        bool housePoolWasPaused = HOUSE_POOL.paused();
        uint8 previousPauseMask;
        if (routerWasPaused) {
            previousPauseMask |= ROUTER_ADMIN_PAUSED_MASK;
        }
        if (housePoolWasPaused) {
            previousPauseMask |= HOUSE_POOL_PAUSED_MASK;
        }
        ...
        emit EmergencyPauseTriggered(
            msg.sender, reasonHash, evidenceHash, riskOffOrderCutoff, previousPauseMask, ALL_COMPONENTS_PAUSED_MASK
        );
    }
