...
/// @notice Calculates maintenance requirement, liquidation-charge split, and residual settlement for a liquidated position.
...
library LiquidationAccountingLib {
    ...
    /// @param lpLiquidationFeeUsdc LP-owned remainder of the total liquidation charge.
    ...
    /// @notice Builds liquidation requirements and splits the collectible charge between keeper and LPs.
    ...
    ///      still produce a minimum charge when collateral is reachable. The keeper receives
    ///      `floor(charge * keeperShareBps / 10_000)` and LPs receive the remainder. A zero `tokenScale` reverts.
    ...
    /// @return state Equity, collateral, maintenance requirement, total charge, keeper bounty, and LP fee.
    function buildLiquidationState(
        ...
        state.lpLiquidationFeeUsdc = liquidationChargeUsdc - state.keeperBountyUsdc;
    }
