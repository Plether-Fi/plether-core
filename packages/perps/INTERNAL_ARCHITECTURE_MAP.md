...
| Execution bounty reserve | Trader-funded keeper reserve until paid or forfeited | `MarginClearinghouse` typed bounty records and `totalBountyReservationsUsdc(account)`, backed by its reserved settlement bucket | `OrderRouter` reserves at commit and routes payout/refund/forfeiture through engine/clearinghouse calls | Pending-order reservation, liquidation reachability, queue liveness review |
| Canonical pool assets (`accountedAssets`, `totalAssets()`) | Protocol-recognized `HousePool` assets | `HousePool` raw/accounted asset ledger | `HousePool` deposit/synchronized-epoch/accountExcess/sweepExcess plus engine-authorized inflow hooks (`recordClaimantInflow(...)`) | Redemption funding, reconciliation, solvency base cash |
...
| `PositionProtectionBook` | Own retained protection state, direct protection actions/views, latest-attempt linkage, and authenticated transfers between clearinghouse bounty namespaces; expose Router-only activation and terminal lifecycle hooks | Custody tokens, directly mutate FIFO, execute keeper/liquidation orchestration, accept a trigger activation from any caller other than its immutable Router, or allow more than one live attempt per protection |
...
- `HousePool.payOut(...)` and `HousePool.recordClaimantInflow(...)` trust only `engine` and `settlementSidecar`; protocol fees remain outside `HousePool` in treasury clearinghouse margin.
...
| Carry realization | Active position margin, then free settlement -> `HousePool` claimant revenue routing | `CfdEngine` via open/close/add-margin and clearinghouse deposit/withdraw hooks | LP-capital carry is projected/collected before health and price settlement; health uses reduced pledge. Carry left uncovered by both margin and free settlement blocks withdrawal and makes the account liquidatable. Other reserves and claims are protected during collection |
