...
| Liquidation accounting | `LiquidationAccountingLib`, liquidation preview/live engine paths | Liquidation-reachable trader value, pending-order reserve exclusions, total-charge cap, keeper/LP split, residual payout, bad debt |
| Solvency / withdrawal cash state | `SolvencyAccountingLib`, degraded-mode checks, protocol accounting snapshot builders | `HousePool.totalAssets()` / net physical assets, bounded max liability, trader claim liabilities, withdrawal reserve, free withdrawable cash |
...
| Liquidation charge | Reachable liquidated-account margin -> keeper clearinghouse account + `HousePool` claimant revenue | `CfdEngine` / `CfdEngineSettlementSidecar` | Splits the capped charge using timelocked `keeperShareBps`; keeper gets the rounded-down configured share and LPs get the remainder without touching pre-existing pool cash |
