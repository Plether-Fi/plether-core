...
| Settlement and live oracle refresh | `OrderRouter` -> immutable Book-carried stateless logic -> `HousePool` | The external entrypoint remains the Router; inside the delegate frame `address(this)` and direct-event address are the Router, and HousePool sees the Router as caller. Request cutoff does not change maturity, phase order, or the post-round-hour publication boundary |
