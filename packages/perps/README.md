...
- Ordinary FIFO orders are binding once committed. Users cannot cancel queued opens, closes, or a close generated by a
  triggered protection. An off-queue `PendingOpen` or `Armed` protection can be cancelled or replaced before it
  triggers.
...
- `PositionProtectionBook`: immutable Router-deployed action/view surface and retained TP/SL lifecycle store. Its
  runtime also carries the Router's stateless mark-refresh/trigger-oracle, single-liquidation, liquidation-batch, and
  LP-epoch orchestration bytecode to keep the Router below the EIP-170 deployment limit.
...
- The Router deploys exactly one Book in its constructor and exposes it through `positionProtectionBook()`. The Router
  ...
  linked-close append.
- Public mark refresh, single liquidation, liquidation batch, and LP-epoch settlement remain Router selectors. Their
  large stateless implementations are reached only by Router `delegatecall` into bytecode carried by the immutable Book.
  That code reads integrations through external Router getters and does not read or write Router storage by layout;
  Router-owned mutations use authorized external self/item calls. Direct calls to those selectors on the Book revert.
...
- Router pause and `positionProtectionCommitsEnabled == false` block new protection creation, replacement, and attached
  opens. They do not strand cancellation, valid triggering, linked-order execution, terminal cleanup, or liquidation.
