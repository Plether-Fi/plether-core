...
- The Router accepts the predeployed lifecycle Book and creates the position-protection Book in its constructor. It
  ...
  FIFO by itself; it invokes narrow Router host paths for the parent open, trigger mark refresh, and linked-close
  append.
...
Every terminal path emits one receipt: success, expiry, slippage, configuration mismatch, typed planner or policy
...
router-policy or oracle-address finalization. These counters are monotonic inputs to the V2 execution-config digest,
so a finalized change invalidates the pinned configuration of an otherwise pending V2 order.
