...
contract TerminalNavBruteForceModelTest is Test {
    ...
    function test_KnownBullPayoffCapsOnlyPositiveLpRecovery() public pure {
        TerminalNavBruteForceModel.AccountState memory state = _state(CfdTypes.Side.BULL, 2, 200, 30);
    ...
    function test_KnownBearPayoffCapsOnlyPositiveLpRecovery() public pure {
        TerminalNavBruteForceModel.AccountState memory state = _state(CfdTypes.Side.BEAR, 2, 200, 30);
    ...
    function test_KnownAccountCapDistributionChangesAggregateValue() public pure {
        ...
        states[0] = _state(CfdTypes.Side.BULL, 1, 100, 10);
        states[1] = _state(CfdTypes.Side.BEAR, 1, 110, 100);
    ...
    function test_KnownBoundariesPreserveEntryDust() public pure {
        TerminalNavBruteForceModel.AccountState memory bull = _state(CfdTypes.Side.BULL, 3, 10, 2);
        TerminalNavBruteForceModel.AccountState memory bear = _state(CfdTypes.Side.BEAR, 3, 10, 2);

        assertEq(TerminalNavBruteForceModel.breakEvenBoundary(bull), 4);
        assertEq(TerminalNavBruteForceModel.accountDeltaAt(bull, 3), -1);
        assertEq(TerminalNavBruteForceModel.accountDeltaAt(bull, 4), 2);

        (bool bullHasTransition, uint256 bullBoundary) =
            TerminalNavBruteForceModel.capTransitionBoundary(bull, CAP_PRICE);
        assertTrue(bullHasTransition);
        assertEq(bullBoundary, 4);

        (bool bearHasTransition, uint256 bearBoundary) =
            TerminalNavBruteForceModel.capTransitionBoundary(bear, CAP_PRICE);
        assertTrue(bearHasTransition);
        assertEq(bearBoundary, 3);
        assertEq(TerminalNavBruteForceModel.accountDeltaAt(bear, 2), 2);
        assertEq(TerminalNavBruteForceModel.accountDeltaAt(bear, 3), 1);
        assertEq(TerminalNavBruteForceModel.accountDeltaAt(bear, 4), -2);
    }
