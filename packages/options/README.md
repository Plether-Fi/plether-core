...
This package depends on `shared` plus the spot-owned `ISyntheticSplitter` API. Build it independently from the repository
root with `forge build --root packages/options` and test it with `forge test --root packages/options`. Its package-owned
tests live under `test/options/`.
