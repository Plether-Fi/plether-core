...
The no-broadcast deployment simulation passed from the existing local test signer. All 23 top-level contract creation inputs fit EIP-3860, including exact constructor arguments; the script prepares 32 total transactions and estimates 0.017378083761769789 test ETH. Fresh-stack broadcast was attempted and interrupted by concurrent nonce usage on the existing signer. One MockUSDC transaction succeeded; no complete perps candidate exists. The canonical inactive-deployment readback remains pending an idle-signer choice. See `manifest.json.deploymentAttempts` for the orphan address, transaction and exact fee. No bootstrap, seeding, activation, migration, consumer cutover or live-state repair was performed by this release work.
