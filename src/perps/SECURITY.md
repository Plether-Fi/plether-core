...
- `OrderRouter` is the external execution boundary and can reach engine settlement paths plus `HousePool.recordProtocolInflow(...)` for forfeited escrow it has already funded.
...
- `HousePool.payOut(...)` trusts only `engine` and `settlementModule` as capability-bearing callers.
