...
- **Single order source of truth**: `getOrderRecord(orderId)` exposes the canonical `OrderRecord` for each order. Its `core`, `status`, residual committed margin, reserved execution bounty, and queue pointers must move together so lifecycle and queue proofs read from one record instead of parallel mappings.
- **Dual-queue structure**: Each account maintains two intrusive FIFO lists. The general pending queue (`pendingHeadOrderId`, `pendingTailOrderId`) contains every live pending order through `OrderRecord.nextPendingOrderId` / `OrderRecord.prevPendingOrderId`. The margin queue (`marginHeadOrderId`, `marginTailOrderId`) contains only live pending orders whose `OrderRecord.remainingCommittedMarginUsdc > 0`, linked through `OrderRecord.nextMarginOrderId` / `OrderRecord.prevMarginOrderId`.
...
- **Dual-queue membership invariant**: An order may appear in the margin queue if and only if `OrderRecord.status == Pending` and `OrderRecord.remainingCommittedMarginUsdc > 0`. Close orders and zero-margin orders must never enter the margin queue, and any order drained to zero residual committed margin must be unlinked from it eagerly.
- **Finalization invariant**: Any path that cancels, fails, or finalizes an order must unlink that order from the general pending queue exactly once, unlink it from the margin queue if present, clear its residual committed margin, clear or distribute its reserved execution bounty, and set the terminal `OrderRecord.status` without erasing immutable order metadata.
- **Consumption invariant**: `noteCommittedMarginConsumed(accountId, amountUsdc)` walks only the margin queue, may only decrease `OrderRecord.remainingCommittedMarginUsdc` for reachable orders in that queue, and must revert if the reachable residual committed margin is insufficient.
- **Queue/accounting agreement**: `pendingOrderCounts[accountId]` should equal the number of orders reachable from `pendingHeadOrderId[accountId]`. Separately, the orders reachable from `marginHeadOrderId[accountId]` should be exactly the pending orders with positive residual `remainingCommittedMarginUsdc`, and `getAccountEscrow(accountId).committedMarginUsdc` should equal the sum of residual `remainingCommittedMarginUsdc` across the full pending queue.
