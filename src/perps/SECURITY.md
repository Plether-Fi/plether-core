...
- **Finalization invariant**: Any path that fails or finalizes an order must unlink that order from the general pending queue exactly once, unlink it from the margin queue if present, release or consume its clearinghouse reservation residual exactly once, clear or distribute its reserved execution bounty, and set the terminal `OrderRecord.status` without erasing immutable order metadata.
