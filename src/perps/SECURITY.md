...
- **Dual-queue structure**: Each account maintains two intrusive FIFO lists. The general pending queue (`pendingHeadOrderId`, `pendingTailOrderId`, `nextPendingOrderId`, `prevPendingOrderId`) contains every live pending order. The margin queue (`marginHeadOrderId`, `marginTailOrderId`, `nextMarginOrderId`, `prevMarginOrderId`) contains only live pending orders whose `committedMargins[orderId] > 0`.
...
- **Dual-queue membership invariant**: An order may appear in the margin queue if and only if it is still pending for that account and `committedMargins[orderId] > 0`. Close orders and zero-margin orders must never enter the margin queue, and any order drained to zero residual committed margin must be unlinked from it eagerly.
- **Finalization invariant**: Any path that cancels, fails, or finalizes an order must unlink that order from the general pending queue exactly once, unlink it from the margin queue if present, clear its residual committed margin, and clear or distribute its reserved execution bounty.
- **Consumption invariant**: `noteCommittedMarginConsumed(accountId, amountUsdc)` walks only the margin queue, may only decrease residual `committedMargins` for reachable orders in that queue, and must revert if the reachable residual committed margin is insufficient.
- **Queue/accounting agreement**: `pendingOrderCounts[accountId]` should equal the number of orders reachable from `pendingHeadOrderId[accountId]`. Separately, the orders reachable from `marginHeadOrderId[accountId]` should be exactly the pending orders with positive residual `committedMargins`, and `getAccountEscrow(accountId).committedMarginUsdc` should equal the sum of residual `committedMargins` across the full pending queue.
