...
**Order Escrow Accounting**: Order escrow now flows through a first-class `OrderEscrowAccounting` module. Router-held execution bounty reserves, clearinghouse-backed committed margin reservations, margin-queue bookkeeping, and account-level escrow summaries now share one canonical implementation boundary instead of being reconstructed separately across router helpers and engine-facing accounting views.
