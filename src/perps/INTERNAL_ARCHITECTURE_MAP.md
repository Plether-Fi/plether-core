...
| trader claim balance | Traders owed realized close proceeds | `MarginClearinghouse` non-spendable trader claim balance; cash remains in `HousePool` until paid | `CfdEngine` records on illiquid profitable close and services claims as cash becomes available | Withdrawal reserves, reconciliation, solvency |
| keeper claim balance | Liquidation keepers owed unpaid value | `MarginClearinghouse` non-spendable keeper claim balance; cash remains in `HousePool` until paid | `CfdEngine` records on illiquid liquidation and services claims as cash becomes available | Withdrawal reserves, reconciliation, solvency |
...
|------|-------------------|------------------------------|
| Close accounting | `CloseAccountingLib`, close preview/live engine paths | Free settlement, released position margin, realized fees, trader payout, bad debt, trader claim balance fallback |
...
| Solvency / withdrawal cash state | `SolvencyAccountingLib`, degraded-mode checks, fee-withdraw gate, protocol accounting snapshot builders | `HousePool.totalAssets()` / net physical assets, bounded max liability, non-spendable claim liabilities, protocol fees, withdrawal reserve, free withdrawable cash |
| Reconciliation / NAV | `HousePoolAccountingLib.buildReconcileSnapshot()` | Net physical assets, unrealized MtM liability only, protocol fees, non-spendable claim liabilities, tranche principal / HWM, unassigned assets |
...
| Profitable close | Position margin + free settlement + vault cash -> trader settlement or trader claim balance | `CfdEngine` / `CfdEngineSettlementModule` | Realizes trader claim; may create a senior vault claim liability instead of reverting |
...
| Liquidation bounty | Reachable trader value first, then engine-serviced vault cash or keeper claim balance | `CfdEngine` | Pays or records the keeper claim without overstating reachable collateral |
