...
| Realized carry revenue | LP-owned trading revenue sourced from trader capital rent | `HousePool` canonical trading-revenue inflow plus `CfdEngine` carry realization paths | `CfdEngine` realizes on open/close/add-margin/withdraw-margin and records via `recordTradingRevenueInflow` | LP revenue, reconciliation, solvency |
...
| Carry realization | Trader reachable capital on realizing actions -> `HousePool` trading revenue | `CfdEngine` | Time-based LP-capital rent becomes LP trading revenue without a separate liquidation settlement path |
