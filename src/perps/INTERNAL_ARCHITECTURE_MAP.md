...
| Carry realization | Trader reachable capital on realizing actions -> `HousePool` trading revenue | `CfdEngine` via open/close/add-margin and clearinghouse deposit/withdraw hooks | Time-based LP-capital rent becomes LP trading revenue without a separate liquidation settlement path |
