...
| `OrderRouter` | Convert trader balance into queued committed margin and execution-bounty escrow; advance or unwind order lifecycle | Mutate `HousePool` accounting directly or invent trader/pool economics outside engine-validated outcomes |
...
- `HousePool.payOut(...)` and `HousePool.recordProtocolInflow(...)` trust `engine`, `orderRouter`, and `settlementSidecar` as high-authority callers.
...
| Liquidation bounty | Reachable trader value first, otherwise pool cash or deferred bounty queue | `CfdEngine` | Pays or records the keeper claim without overstating reachable collateral |
