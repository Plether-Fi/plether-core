...
| Execution-bounty reservation | Trader-funded keeper escrow until paid or forfeited | `MarginClearinghouse.reservedSettlementUsdc` plus router order records | `OrderRouter` reserves at commit and instructs engine/clearinghouse credit or forfeiture on terminal lifecycle events | Pending-order escrow, liquidation reachability, queue liveness review |
...
| Keeper bounty credit | Order/liquidation keepers owed bounty value | Direct `MarginClearinghouse` settlement credit funded from reserved trader margin or liquidation-reachable collateral | `CfdEngine` credits through `MarginClearinghouse.transferReservedSettlement(...)` or liquidation settlement | Keeper settlement, queue liveness review |
...
- `OrderRouter` is the main external execution boundary: it can drive engine settlement paths, but it cannot call `HousePool.payOut(...)` or `recordProtocolInflow(...)`.
...
- `HousePool.payOut(...)` and `HousePool.recordProtocolInflow(...)` trust only `engine` and `settlementSidecar` as high-authority callers.
...
| Commit open order | Free settlement -> committed margin + router bounty escrow | `OrderRouter` via clearinghouse | Moves trader cash into pending-order escrow; no HousePool effect |
...
| Profitable close | Position margin + free settlement + pool cash -> trader settlement or deferred trader credit | `CfdEngine` / `CfdEngineSettlementSidecar` | Realizes trader claim; may create deferred senior pool liability instead of reverting |
| Losing close / collectible carry / liquidation seizure | Trader reachable balance -> `HousePool` accounted inflow | `CfdEngine` | Realized trader loss becomes physical pool cash, then routes as protocol fee, trading revenue, or recapitalization by source semantics |
| Liquidation bounty | Liquidated account margin -> keeper clearinghouse credit | `CfdEngine` / `MarginClearinghouse` | Pays the keeper from reachable trader value without touching pool cash or creating deferred claims |
