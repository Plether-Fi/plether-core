...
|-------|----------------|---------------------|
| `MarginClearinghouse` | Custody trader settlement USDC, lock/release reserved buckets, and settle balances under trusted engine/router calls | Reprice the vault, classify LP ownership, or pay arbitrary third parties |
...
- `MarginClearinghouse` broad settlement paths trust only `engine` and `settlementModule`; `orderRouter` is limited to queued-order reservation lifecycle calls.
- `HousePool.payOut(...)` trusts `engine` and `settlementModule`; protocol fees remain outside `HousePool` in treasury clearinghouse margin.
