...
| Deferred trader payouts | Traders owed realized close proceeds | Queue state in `CfdEngine.deferredPayoutUsdc` and oldest-first claim queue; cash remains in `HousePool` until paid | `CfdEngine` records on illiquid profitable close and services claims when cash is available | Withdrawal reserves, reconciliation, solvency |
