...
- `OrderRouter` is the main external capability boundary: it can drive engine settlement paths and, through approved caller checks, reach `HousePool.payOut(...)`. Router-sourced protocol inflows physically fund the clearinghouse and credit the treasury account directly.
...
- `HousePool.payOut(...)` trusts `engine`, `orderRouter`, and `settlementModule`; `HousePool.recordProtocolBackingInflow(...)` remains for non-fee pool inflows and does not custody protocol fees in the treasury-margin model.
...
| Execute open | Committed margin -> live position margin; protocol fee -> treasury clearinghouse account; adverse trading cash -> `HousePool` accounted inflow when realized | `CfdEngine` | Converts pending escrow into live exposure; fee value becomes treasury margin while non-fee trading inflows become canonical pool cash |
...
| Router forfeiture on liquidation cleanup | Router bounty escrow -> treasury clearinghouse account | `OrderRouter` -> `MarginClearinghouse.settleUsdc(...)` | Converts abandoned queued-order escrow into protocol-owned clearinghouse margin |
