...
contract OptionToken {
    ...
    modifier onlyEngine() {
        if (msg.sender != marginEngine) revert OptionToken__Unauthorized();
        _;
    }

    /// @notice Initialize the proxy (called once by MarginEngine).
    function initialize(string memory _name, string memory _symbol, address _marginEngine) external {
        if (_initialized) revert OptionToken__AlreadyInitialized();
        name = _name;
        symbol = _symbol;
    ...
    function burn(address from, uint256 amount) external onlyEngine {
        if (balanceOf[from] < amount) revert OptionToken__InsufficientBalance();
    ...
    function transfer(address to, uint256 amount) external returns (bool) {
        if (balanceOf[msg.sender] < amount) revert OptionToken__InsufficientBalance();
    ...
    function transferFrom(address from, address to, uint256 amount) external returns (bool) {
        if (allowance[from][msg.sender] != type(uint256).max) {
            if (allowance[from][msg.sender] < amount) revert OptionToken__InsufficientAllowance();
