...
def main():
    ...
    if call('eth_getCode', [PYTH, block['number']]) == '0x':
        raise ValueError('Supported Pyth deployment has no code')
    ...
    if min(stored) >= min(times):
        raise ValueError('Pinned storage cannot reproduce the ordering mismatch; capture another fixture')
