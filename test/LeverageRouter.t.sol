...
contract LeverageRouterTest is Test {
    ...
    function test_OpenLeverage_Success() public {
        ...
        router.openLeverage(1000 * 1e6, 3e18, 100, 0, block.timestamp + 1 hours);
        vm.stopPrank();
    ...
    function test_CloseLeverage_Success() public {
        ...
        // Now Close (router queries actual debt from Morpho)
        morpho.setAuthorization(address(router), true);
        router.closeLeverage(3000 * 1e18, 100, 0, block.timestamp + 1 hours);
        vm.stopPrank();
    ...
    function test_CloseLeverage_ZeroDebt_PartialWithdraw() public {
        ...
        // Withdraw only 1000 of 3000 collateral (no debt to repay)
        router.closeLeverage(1000 * 1e18, 100, 0, block.timestamp + 1 hours);
        vm.stopPrank();
    ...
    function test_PreviewCloseLeverage_ZeroReturn() public view {
        ...
        (,, uint256 expectedReturn) = router.previewCloseLeverage(10_000 * 1e6, 100 * 1e18);
    ...
    function test_OpenLeverage_ZeroPrincipal_Reverts() public {
        ...
        router.openLeverage(0, 3e18, 100, 0, block.timestamp + 1 hours);
        vm.stopPrank();
    }
    ...
    function test_OpenLeverage_LeverageAtMinimum_Reverts() public {
        ...
        vm.expectRevert(LeverageRouterBase.LeverageRouterBase__LeverageTooLow.selector);
        router.openLeverage(1000 * 1e6, 1e18, 100, 0, block.timestamp + 1 hours);
        vm.stopPrank();
    }
    ...
    function test_OpenLeverage_LeverageTooLow_Reverts() public {
        ...
        vm.expectRevert(LeverageRouterBase.LeverageRouterBase__LeverageTooLow.selector);
        router.openLeverage(1e6, 1e18 + 100, 100, 0, block.timestamp + 1 hours);
        vm.stopPrank();
    }
    ...
    function test_OpenLeverage_Deadline_Reverts() public {
        ...
        router.openLeverage(1000 * 1e6, 3e18, 100, 0, block.timestamp - 1);
        vm.stopPrank();
    }
    ...
    function test_CloseLeverage_Deadline_Reverts() public {
        ...
        vm.expectRevert(LeverageRouterBase.LeverageRouterBase__Expired.selector);
        router.closeLeverage(3000 * 1e18, 100, 0, block.timestamp - 1);
        vm.stopPrank();
    }
    ...
    function test_OpenLeverage_SlippageExceedsMax_Reverts() public {
        ...
        router.openLeverage(1000 * 1e6, 3e18, 200, 0, block.timestamp + 1 hours); // 200 bps > 100 max
        vm.stopPrank();
    }
    ...
    function test_CloseLeverage_SlippageExceedsMax_Reverts() public {
        ...
        vm.expectRevert(LeverageRouterBase.LeverageRouterBase__SlippageExceedsMax.selector);
        router.closeLeverage(3000 * 1e18, 200, 0, block.timestamp + 1 hours); // 200 bps > 100 max
        vm.stopPrank();
    }
    ...
    function test_OpenLeverage_ExtremeLeverage_100x() public {
        ...
        router.openLeverage(1000 * 1e6, 100e18, 100, 0, block.timestamp + 1 hours);
        vm.stopPrank();
    ...
    function test_OpenLeverage_MinimalLeverage_1_1x() public {
        ...
        router.openLeverage(1000 * 1e6, 1_100_000_000_000_000_000, 100, 0, block.timestamp + 1 hours);
        vm.stopPrank();
    ...
    function test_OpenLeverage_SucceedsWithMinimalPrincipal() public {
        ...
        router.openLeverage(1, 3e18, 100, 0, block.timestamp + 1 hours);
        vm.stopPrank();
    ...
    function test_OpenLeverage_SmallPrincipal_1000Wei() public {
        ...
        router.openLeverage(1000, 3e18, 100, 0, block.timestamp + 1 hours);
        vm.stopPrank();
    ...
    function testFuzz_CloseLeverage(
        ...
        // Bound inputs
        principal = bound(principal, 1000e6, 100_000e6);
    ...
    function testFuzz_OpenLeverage_VariableBearPrice(
        ...
        bearPriceBps = bound(bearPriceBps, 5000, 20_000); // $0.50 to $2.00 per BEAR

        // Set BEAR price (in 6 decimals for mock)
        uint256 bearPrice = (bearPriceBps * 1e6) / 10_000;
    ...
    function testFuzz_OpenLeverage_SlippageTolerance(
        ...
        router.openLeverage(principal, 3e18, slippageBps, 0, block.timestamp + 1 hours);
        vm.stopPrank();
    ...
    function test_AddCollateral_Success() public {
        ...
        usdc.approve(address(router), 1000 * 1e6);
        morpho.setAuthorization(address(router), true);
        router.openLeverage(1000 * 1e6, 2e18, 100, 0, block.timestamp + 1 hours);
        ...
        router.addCollateral(500 * 1e6, 100, 0, block.timestamp + 1 hours);
        vm.stopPrank();
    ...
    function test_AddCollateral_Expired_Reverts() public {
        ...
        vm.expectRevert(LeverageRouterBase.LeverageRouterBase__Expired.selector);
        router.addCollateral(500 * 1e6, 100, 0, block.timestamp - 1);
        vm.stopPrank();
    }
    ...
    function test_RemoveCollateral_Success() public {
        ...
        morpho.setAuthorization(address(router), true);
        router.removeCollateral(1000 * 1e18, 100, 0, block.timestamp + 1 hours);
        vm.stopPrank();
...
contract MockCurvePool is ICurvePool {
    ...
    function exchange(
        ...
        dy = (quotedDy * (10_000 - slippageBps)) / 10_000; // Apply slippage
...
contract MockMorpho is IMorpho {
    ...
    function position(
        ...
        // We use borrowBalance as borrowShares for simplicity (1:1 ratio)
    ...
    function market(
        ...
        // For simplicity, use 1:1 ratio for shares:assets
        uint128 totalBorrow = 0;
        // Sum all borrows (simplified - in real tests we'd track this properly)
        // For now just return a large enough value that division works
...
contract MockStakedTokenWithOffset is ERC20 {
    ...
    constructor(
        address _underlying
    ) ERC20("Staked Token With Offset", "sTKN") {
        underlying = MockToken(_underlying);
    }
...
contract LeverageRouterOffsetTest is Test {
    ...
    function test_CloseLeverage_WithOffset_Success() public {
        ...
        // Close leverage (router queries actual debt from Morpho)
        morpho.setAuthorization(address(router), true);
        router.closeLeverage(shares, 100, 0, block.timestamp + 1 hours);
