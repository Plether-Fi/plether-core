...
contract OrderRouterTest is BasePerpTest {
    ...
    function test_ZeroSizeCommit_Reverts() public {
        vm.prank(alice);
        vm.expectRevert(IOrderRouter.OrderRouter__ZeroSize.selector);
    ...
    function test_ExecuteNonPendingOrder_Reverts() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IOrderRouter.OrderRouter__QueueState.selector, 0));
        vm.roll(10);
        router.executeOrder(1, empty);
    }
    ...
    function test_StrictFIFO_OutOfOrder_Reverts() public {
        ...
        bytes[] memory empty;
        vm.expectRevert(abi.encodeWithSelector(IOrderRouter.OrderRouter__QueueState.selector, 1));
        vm.roll(10);
    ...
    function test_CommitOrder_RevertsWhenPendingOrderCountHitsCap() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IOrderRouter.OrderRouter__CommitValidation.selector, 7));
        router.commitOrder(CfdTypes.Side.BULL, 1000e18, 100e6, 1e8, false);
        vm.stopPrank();
    }
    ...
    function test_BatchExecution_NoOrders_Reverts() public {
        bytes[] memory empty;
        vm.expectRevert(abi.encodeWithSelector(IOrderRouter.OrderRouter__QueueState.selector, 2));
        vm.roll(block.number + 1);
    ...
    function test_BatchExecution_UncommittedMaxId_Reverts() public {
        ...
        bytes[] memory empty;
        vm.expectRevert(abi.encodeWithSelector(IOrderRouter.OrderRouter__QueueState.selector, 3));
        vm.roll(block.number + 1);
...
contract OrderRouterPythTest is BasePerpTest {
    ...
    function test_PythConfidenceTooWide_RevertsExecution() public {
        ...
        vm.roll(block.number + 1);
        vm.expectRevert(abi.encodeWithSelector(IOrderRouter.OrderRouter__OracleValidation.selector, 11));
        router.executeOrder(1, _pythUpdateData());
    }
    ...
    function test_CommitOrder_RevertsOnPredictableMustCloseOpposing() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IOrderRouter.OrderRouter__PredictableOpenInvalid.selector,
    ...
    function test_CloseCommit_RevertsWhenPendingCloseSizeWouldExceedPosition() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IOrderRouter.OrderRouter__CommitValidation.selector, 5));
    ...
    function test_StateMachine_StaleRevertPreservesQueueUntilHonestBatchExecutes() public {
        ...
        address account = alice;
        IOrderRouterAccounting.AccountEscrowView memory beforeEscrow = router.getAccountEscrow(account);
        ...
        IOrderRouterAccounting.AccountEscrowView memory finalEscrow = router.getAccountEscrow(account);
    ...
    function test_InsufficientPythFee_Reverts() public {
        ...
        vm.warp(1050);
        vm.expectRevert(abi.encodeWithSelector(IOrderRouter.OrderRouter__OracleValidation.selector, 6));
        vm.roll(block.number + 1);
    ...
    function obsolete_test_Slippage_OpenDirections() public {
        ...
        usdc.approve(address(clearinghouse), type(uint256).max);
        clearinghouse.deposit(trader2, 10_000 * 1e6);
        vm.stopPrank();
...
contract OrderRouterBlockedExecutionTest is BasePerpTest {
    ...
    function obsolete_test_FadWindow_OpenOrderStaysPendingAtExecution() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IOrderRouter.OrderRouter__CommitValidation.selector, 10));
    ...
    function computeBasketPrice(
        ...
        for (uint256 i = 0; i < pythFeedIds.length; i++) {
            ...
            if (p.publishTime > block.timestamp || block.timestamp - p.publishTime > maxStaleness) {
                revert IOrderRouter.OrderRouter__OracleValidation(10);
            }
        ...
        if (basketPrice == 0) {
            revert IOrderRouter.OrderRouter__OracleValidation(8);
        }
...
contract InversionTest is Test {
    ...
    function test_H03_InversionsLengthMismatchReverts() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IOrderRouter.OrderRouter__OracleValidation.selector, 1));
...
contract OrderRouterAuditTest is BasePerpTest {
    ...
    function test_MissingChainIdGuard() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IOrderRouter.OrderRouter__OracleValidation.selector, 4));
        vm.roll(block.number + 1);
        router.executeOrder(1, empty);
    }
...
contract MarkPriceStalenessTest is BasePerpTest {
    ...
    function test_Constructor_ZeroEngineLensReverts() public {
        vm.expectRevert(abi.encodeWithSelector(IOrderRouter.OrderRouter__OracleValidation.selector, 7));
        new OrderRouter(
...
contract KeeperFeeRefundTest is Test {
    ...
    function test_FIFOCleanupImpossibleHeadOrderHasEconomicCleanupIncentive() public {
        ...
        uint256 traderPenaltyUsdc = traderSettlementBefore - clearinghouse.balanceUsdc(account);
        ...
        assertEq(
            router.getAccountEscrow(account).executionBountyUsdc,
            0,
