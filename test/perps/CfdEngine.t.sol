...
contract CfdEngineTest is BasePerpTest {
    ...
    function test_ProtocolTreasuryWithdrawal_DoesNotUseSeniorCashReservation() public {
        ...
        uint256 vaultAssetsBeforeDrain = pool.totalAssets();
        vm.prank(address(pool));
        usdc.transfer(address(0xDEAD), vaultAssetsBeforeDrain);
        ...
        usdc.mint(address(pool), vaultAssetsBeforeDrain);
    }
    ...
    function test_PreviewLiquidation_PreservesLegacyDeferredOnPositivePhysicalResidual() public {
        ...
        uint256 liveEffective = legacySpread > 0
            ? (vaultAssets > uint256(legacySpread) ? vaultAssets - uint256(legacySpread) : 0)
            : vaultAssets + uint256(-legacySpread);
