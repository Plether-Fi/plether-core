...
contract CfdEngineTest is BasePerpTest {
    ...
    function test_ProtocolTreasuryWithdrawal_DoesNotUseSeniorCashReservation() public {
        ...
        assertEq(
            ...
            deferredBounty,
            "Withdrawing treasury margin must not consume deferred senior claims"
        );
    }
    ...
    function test_TreasuryWithdrawal_ThenDeferredClaims_DrainsResidualCashWithoutDeadlock() public {
        ...
        assertEq(pool.totalAssets(), 40e6, "Final deferred claim should leave non-deferred vault cash available");
        assertEq(
            engine.deferredKeeperCreditUsdc(keeper), 0, "Keeper deferred balance should clear after the final claim"
        );
        assertEq(engine.protocolTreasuryBalanceUsdc(), 0, "Treasury accounting should stay outside deferred claims");
    }
    ...
    function test_ClaimDeferredKeeperCredit_UsesFeeOnlyLiquidityWhenAtQueueHead() public {
        ...
        assertEq(
            engine.protocolTreasuryBalanceUsdc(), feesBefore, "Servicing deferred claims must not burn fee accounting"
        );
    }
    ...
    function test_GetProtocolAccountingSnapshot_ReflectsCanonicalLedgerState() public {
        ...
        assertEq(snapshot.netPhysicalAssetsUsdc, snapshot.vaultAssetsUsdc);
    ...
    function test_PlanLiquidation_PositiveResidualAboveDeferredDoesNotUnderflow() public {
        ...
        assertEq(
            delta.keeperBountyUsdc, 0, "No reachable settlement collateral should cap the liquidation bounty at zero"
        );
        assertEq(delta.residualUsdc, 8e6, "Residual should reflect equity when no bounty is payable");
    ...
    function test_PreviewLiquidation_PreservesLegacyDeferredOnPositivePhysicalResidual() public {
        ...
        assertEq(preview.keeperBountyUsdc, 0, "No reachable settlement collateral should cap the bounty at zero");
        ...
        vm.prank(keeper);
        bytes[] memory priceData = new bytes[](1);
        priceData[0] = abi.encode(uint256(100_000_000));
    ...
    function test_LiquidationPreviewAndPositionView_UseCurrentNotionalThreshold() public {
        ...
        vm.prank(address(router));
        engine.liquidatePosition(account, 110_000_000, vaultDepth, uint64(block.timestamp));
    ...
    function test_SetRiskParams_MakesPositionLiquidatable() public {
        ...
        vm.prank(address(router));
        engine.liquidatePosition(account, 1e8, vaultDepth, uint64(block.timestamp));
        ...
        vm.prank(address(router));
        uint256 bounty = engine.liquidatePosition(account, 1e8, vaultDepth, uint64(block.timestamp));
    ...
    function test_M11_LiquidationSeizesFreeEquity() public {
        ...
        vm.prank(address(router));
        engine.liquidatePosition(account, 1.1e8, vaultDepth, uint64(block.timestamp));
    ...
    function test_LiquidationWorksWhenVaultInsolvent() public {
        ...
        vm.prank(address(router));
        engine.liquidatePosition(aliceAccount, 1.1e8, vaultDepth, uint64(block.timestamp));
    ...
    function test_LiquidationBounty_UsesReachableCollateralSubsidyCap() public {
        ...
        vm.prank(address(router));
        uint256 bounty = engine.liquidatePosition(account, 100_500_000, vaultDepth, uint64(block.timestamp));
    ...
    function test_ClearBadDebt_ReducesOutstandingDebt() public {
        ...
        vm.prank(address(router));
        engine.liquidatePosition(account, 1.2e8, depth, uint64(block.timestamp));
